...
# 第 5 章: 与真实 ToolRegistry 对接，Main Loop 只维护上下文时间线。
...
class AgentEngine:
    ...
    def run(self, user_prompt: str) -> None:
        ...
        # 初始化上下文: System Prompt(人设) + 用户任务
        context_history: list[Message] = [
            Message(role=ROLE_SYSTEM, content="You are py-tiny-claw, an expert "
                    "coding assistant. You have full access to tools in the "
                    "workspace."),
        ...
        while True:
            ...
            try:
                action_resp = self.provider.generate(context_history,
                                                     available_tools)
            except Exception as e:
            ...
            for tool_call in action_resp.tool_calls:
                log.info(" -> ⚒️ 执行工具: %s, 参数: %s", tool_call.name,
                         tool_call.arguments)
